    ...
    <>
        ...
        <div className="page-container flex items-center justify-between h-full gap-4">
          ...
          <div className="hidden md:flex items-center gap-1">
            {NAV_LINKS.slice(1).map(({ href, label }) => (
              <Link
                ...
                href={href}
                className={`
                  text-sm px-3 py-1.5 rounded-[6px] transition-colors font-medium
                  ${isActive(href)
          ...
          <div className="flex items-center gap-1 sm:gap-2">
              ...
              {profile ? (
                <div className="flex items-center gap-2">
                  ...
                  {profile.current_streak > 0 && (
                    <div className="flex items-center gap-1 text-xs font-bold text-orange-500 bg-orange-500/10 px-2.5 py-1 rounded-[6px] border border-orange-500/20">
                  ...
                  <div className="group relative">
                    ...
                    <div className="absolute right-0 top-full w-52 hidden group-hover:block animate-fade-up" style={{ paddingTop: '2px' }}>
                      <div className="bg-[var(--bg-primary)] border border-[var(--border)] rounded-[6px] shadow-2xl overflow-hidden">
                        {/* User identity */}
                ...
                <div className="flex items-center gap-2">
                  <Link href="/auth/signin" className="text-sm font-medium px-3 py-1.5 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors rounded-[6px] hover:bg-[var(--bg-secondary)]">
                    Sign in
                  </Link>
                  <Link href="/auth/signup" className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white text-sm font-semibold px-4 py-1.5 rounded-[6px] transition-colors">
                    Get started
                  </Link>
                </div>
              )}
            ...
            <button
              ...
              className="md:hidden touch-target rounded-[6px] hover:bg-[var(--bg-secondary)] transition-colors flex-shrink-0"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-14 border-b border-[var(--border)] flex-shrink-0">
          ...
          <button
            onClick={() => setDrawerOpen(false)}
            className="touch-target rounded-[6px] hover:bg-[var(--bg-secondary)] transition-colors"
        ...
        {profile && (
          <div className="mx-4 mt-4 p-4 rounded-[6px] bg-[var(--bg-secondary)] border border-[var(--border)]">
            <div className="flex items-center gap-3">
              ...
              {profile.current_streak > 0 && (
                <div className="flex items-center gap-1 text-xs font-bold text-orange-500 bg-orange-500/10 px-2.5 py-1 rounded-[6px] border border-orange-500/20 flex-shrink-0">
                  <Flame className="h-3.5 w-3.5 fill-current" />
                  {profile.current_streak}
                </div>
              )}
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
          {NAV_LINKS.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              onClick={() => setDrawerOpen(false)}
              className={`
                flex items-center gap-3 px-4 py-3 rounded-[6px] text-sm font-medium transition-colors
                ${isActive(href)
            ...
            <>
              ...
              {profile.is_admin && (
                <Link
                  href="/admin"
                  onClick={() => setDrawerOpen(false)}
                  className="flex items-center gap-3 px-4 py-3 rounded-[6px] text-sm font-medium text-[var(--accent)] hover:bg-[var(--accent-subtle)] transition-colors"
                >
        ...
        <div className="px-4 pt-4 space-y-2 flex-shrink-0 border-t border-[var(--border)]">
          {profile ? (
            <button
              onClick={handleSignOut}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-[6px] text-sm font-medium text-rose-500 hover:bg-rose-500/5 transition-colors"
            >
